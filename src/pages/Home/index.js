...
function Home() {
  ...
  const newQuestions = [...questions.answers];
  useEffect(() => {
    setShuffledAnswers(shuffleArray(newQuestions));
  }, [questions]);
  ...
  return (
    ...
    >
      {isStarted ? (
          ...
          {questionCounter <= 10 ? (
            <div>
              ...
              <div className="soru">
                ...
                <div className="sorutext">{`${questions?.num1} x ${questions?.num2}`}</div>
              </div>
              <div className="answers">
                <ul className={background}>
                  {shuffledAnswers?.map((answer, idx) => (
                    <li className={`${background}`} key={idx}>
                      <button
                        className={`answerButton answersButton ${background}`}
                        disabled={
                          background === "answerCorrect" ||
                          background === "answerNotCorrect"
                            ? true
                            : false
                        }
            ...
            >
              ...
              >
                ...
                {correctAnswerList.map((answerList, idx) => (
                  ...
                  >{`${answerList.num1} x ${answerList.num2} = ${answerList.answer} ${answerList.answerStatus} `}</div>
                ))}
              </div>
            </div>
          )}
        ...
        <div className="mathshome">
          <div>
            <h1 className="text">Mathematics Game</h1>
          </div>
